...
with 
	faixa_de_idade_dos_veiculos as (
		...
		select
			...
			case
				when (extract('year' from visit_page_date) - prod.model_year::int) <= 2 then '1'
				when (extract('year' from visit_page_date) - prod.model_year::int) <= 4 then '2'
				when (extract('year' from visit_page_date) - prod.model_year::int) <= 6 then '3'
				when (extract('year' from visit_page_date) - prod.model_year::int) <= 8 then '4'
				when (extract('year' from visit_page_date) - prod.model_year::int) <= 10 then '5'
				else 6
				end as "ordem"
